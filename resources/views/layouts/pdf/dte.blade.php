    ...
    <body style="margin: 10.8cm 0.5cm {{ $orientacionPagina === 2 ? 0.5 : 0.4 }}cm 0.5cm;">{{--Margenes del contenido--}}
        ...
        <header>
            <div class="row mr-03 ml-05">
                ...
                <div class="col-3">
                    <p class="text-right mt-15px">VERSIÓN @yield('version', '0')</p>
                </div>
            </div>
            ...
            <div class="row ml-02 mr-n06 mt-2">
                <div style="padding: 0 0.3cm;" class="col-5">
                    ...
                    <p class="mt-n015">{{ $dte->codigo_generacion }}</p>
                    <p class="mt-n015"><b>NÚMERO DE CONTROL:</b></p>
                    <p class="mt-n015">{{ $dte->correlativo }}</p>
                    <p class="mt-n015"><b>SELLO DE RECEPCIÓN:</b></p>
                    <p class="mt-n015">{{ $dte->sello_recibido }}</p>
                </div>
                <div class="col-1"></div>{{--Columna vacia--}}
                <div style="padding: 0 0.1cm;" class="col-5">
                    <p><b>MODELO DE FACTURACIÓN:</b></p>
                    <p class="mt-n015">{{ $utils->modeloFacturacion($json->identificacion->tipoModelo) }}</p>
                    <p class="mt-n015"><b>TIPO DE TRANSMISIÓN:</b></p>
                    <p class="mt-n015">{{ $utils->tipoTransmision($json->identificacion->tipoOperacion) }}</p>
                    <p class="mt-n015"><b>FECHA Y HORA DE GENERACIÓN:</b></p>
                    <p class="mt-n015">{{ $dte->fecha_procesamiento }}</p>
                </div>
            </div>
            ...
            @if($dte->invalidado && $dte->invalidado->id > 0 && strlen($dte->invalidado->sello_recibido) > 30)
                ...
                <div class="text-center">
                    ...
                    @if(strlen($dte->invalidado->codigo_generacion_r) > 30)
                        <p>
                            ...
                            <span class="fs-10pt">{{ $dte->invalidado->codigo_generacion_r }}</span>
                        </p>
                        @if($rqr)
                            <p><img src="{{ $rqr }}" alt="Codigo QR" class="imgQR"></p>
                        @endif
                    @endif
                </div>
            @endif
