...
@section('panel_reporte_eventos')
    <div id="appReporteEventoSonido" class="container">

            <div class="card-body p-3">
                <form action="{{ route('eventos.reporte_sonidos_buscar') }}" method="post">
                    @csrf
                    <div class="row mb-2">
                        <div class="card-title col-12 h5 text-uppercase mb-4">
                            REPORTE DE MONTAJES DE SONIDOS
                        </div>
                        ...
                        <div class="col-4">
                            <div class="mb-3">
                                ...
                                <select name="sonido" class="form-control" value="{{ $sonido ?? '' }}">
                                    <option value="" selected>Todos los sonidos</option>
                                    @foreach ($sonidos as $s)
                                        <option value="{{ $s->id }}"
                                            {{ $sonido && $sonido == $s->id ? 'selected' : '' }}>
                                            {{ $s->sonido }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <table
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">OBSERVACION SONIDO</th>
                                    ...
                                    @endphp
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-uppercase">
                                                {{ $r->sonidos->sonido ?? 'No es requerido' }}
                                            </td>
                                            <td class="text-uppercase">
                                                {{ $r->observaciones_sonidos ?? 'No es requerido' }}
                                            </td>
                                            <td class="text-uppercase">
