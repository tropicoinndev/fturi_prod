...
@section('form')
    <form action="{{ route($route ?? $th['table'] . '.deletePeriodosCreditos') }}" method="post">
        @csrf
        <input type="hidden" name="id" value="{{ \Crypt::encryptString($p->id) }}">
        <div class="row">
            <div class="col-12 h4 mt-2">
                <span class="mdi mdi-alert text-danger"></span> ¿Esta seguro de borrar el periodo de crédito ({{ $p->periodosCreditos->periodo }} · {{ $p->periodosCreditos->dias }}) ?
            </div>
