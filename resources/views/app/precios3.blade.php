    ...
    <div id="appPreciosMovil" class="container-fluid">
        <div class="row justify-content-center">
            ...
            <section :class="{'initBusqueda': !scrollInit && busquedaShow}" id="panel-main" class="mb-5"><!--Margen inferior entre la ultima card y el panel inferior de los botones-->
                ...
                <div v-if="txtSearch.length > 2 && Object.keys(productosList).length > 0" id="abecedarioDiv" class="row mt-3 mb-4">
                    <div class="col-12">
                ...
                <p v-if="txtSearch.length > 2 && Object.keys(productosList).length > 0" style="color: #546E7A; font-weight: 500; font-size: 10.5pt;" class="text-uppercase mt-2">Seleccione Un Producto</p>
                
                {{-- @{{ productosList }} --}}
                <div id="panel-main-opacity"><!--Se usa para la opacidad-->
                    <div class="row">
                        <!--Listado de productos-->
                        <div v-if="txtSearch.length > 2 && Object.keys(productosList).length > 0">
                            ...
                            <div style="margin-top: -25px; border-bottom: solid 5px rgba(84, 110, 122, 51%);" class="panel-card-modal p-3 mb-3">
                                <div>
                                    ...
                                    <div v-show="!ocultarCantidad">
                                        <p style="color: #546E7A; margin-bottom: 2px;">Cantidad: @{{ precioSelected.existencias ?? '∞' }}<span v-if="precioSelected.lote && precioSelected.vencimiento"> · LOTE #@{{ precioSelected.lote }} · Ven. @{{ precioSelected.vencimiento }}</span></p>
                                    ...
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-6 text-end">
                                            <button @click="funcAddProduct" :disabled="txtCantidad <= 0 || lblError" style="background: #DAE0E5; color: #546E7A;" type="button" class="btn"><i class="bi bi-plus-lg"></i> Agregar</button>
                                        </div>
                                    </div>

                                    <p v-show="messageSuccess" style="margin-bottom: 2px;" class="text-success">@{{ messageSuccess }}</p>
                                </div>
                                {{-- <div v-show="!showFormModal" class="text-uppercase text-center">
    ...
    <script type="module">
        let app = window.appVue({
            ...
            mounted(){
                ...
                //this.obtenerProductosSinExistencia();
            ...
            methods: {
                ...
                setPrecio(pre){
                    ...
                    this.precioSelected.id      = pre.cid;
                    this.precioSelected.detalle = pre.detalle;
                    this.precioSelected.precio  = pre.precio;

                    this.precioSelected.existencias = null;
                    this.precioSelected.lote        = null;
                    this.precioSelected.vencimiento = null;

                    switch(pre.token){
                        case 1101://Productos con existencias
                            this.lblNombreBodega = 'SALIDA DE '+this.bodegaSelected.name;
                            this.precioSelected.existencias = pre.existencia;
                            this.precioSelected.lote        = pre.lote;
                            this.precioSelected.vencimiento = pre.ven;
                        break;
                        case 1105://Productos sin existencias
                            this.lblNombreBodega = 'NO REQUIERE EXISTENCIAS.';
                            //this.precioSelected.existencias = '∞';
                        break;
                    }

                    //this.showFormModal = true;
                },
                obtenerProductosSinExistencia(){
                    axios.post("{{ route('precios.apiGetProductos') }}",{
                        busqueda: this.txtSearch,
                    }).then((r) => {
                        this.productosList = r.data.list_5;
                    }).catch((err) => {
                        console.log('Error JS: ',err);
                    });
                },
                validarCantidad(){
                    if(this.txtCantidad <= 0 || this.txtCantidad > this.precioSelected.existencias){
                ...
                funcAddProduct2(){
                    axios.post("{{ route('precios.addProductoApp') }}",{
                        ...
                        precios_id:    this.precioSelected.id,//Id encriptado
                ...
                funcAddProduct(){
                    axios.post("{{ route('detalle_comandas.storeApp') }}",{
                        ...
                        if(r.data.status){
                            //window.location.href = window.location.pathname;//Recargamos esta misma pagina como la propiedad 'pathname'
                            this.contadorProductos += this.txtCantidad;
