            ...
            @can('cajas.estado_cuentas')
                @if (!$p->facturada || $p->estado)
                    <a class="btn btn-light"
                        href="{{ route('cajas.estado_cuentas', ['tipo' => Crypt::encryptString($tipo), 'id' => $p->id]) }}"
                        role="button"
        ...
        <table class="table table-light">
            ...
            <tbody>
                @php
                    $total = 0;
                @endphp
                @forelse ($p->detalle_orden as $d)
                    @php
                        $monto = round($d->precio_unitario * $d->cantidad, 2);
                        $total += $monto;
                    @endphp
                    <tr>
                        <td>{{ $d->cantidad }}</td>
                        <td>{{ $d->servicios->servicio }}</td>
                        <td>${{ number_format($d->precio_unitario, 2) }}</td>
                        <td>${{ number_format($monto, 2) }}</td>
                        <td>{{ $d?->user_detalle?->name }}</td>
                    </tr>
                @empty
                    <tr>
